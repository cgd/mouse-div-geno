    ...
    mchr)
{
    ...
    for (sampleid in 1:ncol(baf)) {
        ...
        colnames(s) <- c(
            "Name",
            paste(gsub("CEL", "Log R Ratio", filenames[sampleid]), sep = "", collapse = ""),
            paste(gsub("CEL", "B Allele Freq", filenames[sampleid]), sep = "", collapse = ""))
        xname1 <- paste(
            cnvoutfiledir,
            "/cnv",
            gsub(".CEL", ".txt", filenames[sampleid]), sep = "", collapse = "")
                ...
                if (s == 1) 
                  cnvtable <- rbind(
                      ...
                      c(
                          ...
                          g[ep, 2],
                          g[ep, ] - g[sp, ] + 1,
                          sum(id[g[sp, 1]:g[ep, 1]] == 1),
                          sum(id[g[sp, 1]:g[ep, 1]] == 2),
                          sum(id[g[sp, 1]:g[ep, 1]] == 3),
                          round(mean(ref[g[sp, 1]:g[ep, 1]]), 3),
                          round(mean(inte[g[sp, 1]:g[ep, 1]]), 3)))
                if (s > 1) {
                  k <- cbind(
                      g[sp, 2],
                      g[ep, 2],
                ...
                if (s == 1) 
                  cnvtable <- rbind(
                      ...
                      c(
                          ...
                          l[ep, 2],
                          l[ep, ] - l[sp, ] + 1,
                          sum(id[l[sp, 1]:l[ep, 1]] == 1),
                          sum(id[l[sp, 1]:l[ep, 1]] == 2),
                          sum(id[l[sp, 1]:l[ep, 1]] == 3),
                          round(mean(ref[l[sp, 1]:l[ep, 1]]), 3),
                          round(mean(inte[l[sp, 1]:l[ep, 1]]), 3)))
                if (s > 1) {
                  k <- cbind(l[sp, 2], l[ep, 2], l[ep, ] - l[sp, ] + 1, l[sp, 1], l[ep, 1])
                  k1 <- apply(
                      k,
                      1,
                      function(x, ref, id, inte) {
