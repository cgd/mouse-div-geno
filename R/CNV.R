pennCNVinput = function(chrid, mpos, exon1info, exon2info, celfiledir, 
    ...
    allbaf = allllr = allpos = chrid = NULL
    for (chri in mchr) {
...
simpleCNV = function(allid, ABid, chrid, CGFLcorrection, reference, exon1info, 
    exon2info, celfiledir, filenames, cnvoutfiledir, mchr) {
    library("affyio")
    library("preprocessCore")
    setwd(celfiledir)
...
simplehmm = function(cnvoutfiledir, filenames, refid, mchr, stname) {
