...
#
#########################################################################
MouseDivGenotype = function(celfiledir, outfiledir, allid, ABid, chrid, CGFLcorrection=NULL, reference=NULL, hint=NULL,
  trans=c("CCStrans","MAtrans"), celnamefile =NULL, mchr = c(1:19, 'X', 'Y', 'M') , subset = FALSE){
  library(affyio); library('preprocessCore'); library(cluster)
  trans = match.arg(trans); print(trans)
  ...
  iiy = !is.na(match('Y', mchr)); iix = !is.na(match('X', mchr));
  if( iiy | iix ) mchr1 = unique(c(mchr, 'X', 'Y'))
  for(i in 1:nfile){
  ...
  autoint = NULL; pseudoautosomalID = c(21845:21870); 
  mchr1 = c(1:19, 'M'); ii=match(mchr1, mchr); mchr1 = mchr1[ !is.na( ii ) ]
  for(chri in mchr1){
    MM = SS = NULL 
    for(i in 1:nfile){
  ...
  if( iix | iiy ){
    ...
    if( length(gender) < nfile ) isMale = computegender(intX, intY, autoint) 
      ...
      if( length(gender1 == 1) > 1 & length(gender1 == 2) > 1 ){ 
        gender = vdist( intY, intX, gender1 ); isMale = gender == 1}
    ...
    if(iiy) genotypethis(outfiledir, MMy, SSy, NULL, isMale, trans, 'Y', pseudoautosomalID)
  }
}
