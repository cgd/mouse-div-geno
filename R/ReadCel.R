...
ReadCelFile = function(celfiledir, outfiledir, allid, ABid, chrid, CGFLcorrection = NULL, 
    reference = NULL, trans = c("CCStrans", "MAtrans"), celnamefile, subset = FALSE) {
    library(affyio)
...
genotypethis = function(savefiledir, MM, SS, hint, isMale, trans, chr, 
    ...
    if (!is.na(autosome)) {
        conf = geno = vino = NULL
        subset = seq(1, nn, 2000)
        sp = subset
        lls = length(sp)
        ep = c(subset[2:lls] - 1, nn)
        for (iter in 1:lls) {
            tmp = sp[iter]:ep[iter]
    ...
    if (!is.na(chrX)) {
        ...
        lp = length(pseudoautosomalID)
        id = c(1:nn)
        if (lp > 0) 
            id = setdiff(c(1:nn), pseudoautosomalID)
        subset = seq(1, length(id), 2000)
        sp = subset
        lls = length(sp)
        ep = c(subset[2:lls] - 1, length(id))
        n = sum(!isMale)
        if (n > 1) {
        ...
        if (n > 1) {
            ...
            for (iter in 1:lls) {
                ...
                genom1 = apply(cbind(sMM[tmp, ], sSS[tmp, ]), 1, function(x, trans) {
                  b = genotypeSexchr(x[1:n], x[(n + 1):(2 * n)], trans)
                  c(b$geno, b$vino, b$conf)
                }, trans)
                genom1 = t(genom1)
                confm = rbind(confm, genom1[, c((2 * n + 1):(3 * n))])
        ...
        if (lp > 1) {
            confm = genom = vinom = NULL
            genom = apply(cbind(MM[pseudoautosomalID, ], SS[pseudoautosomalID, ], 
                hint[pseudoautosomalID]), 1, function(x, trans) {
                b = genotype(x[1:n], x[(n + 1):(2 * n)], x[(2 * n + 1):(2 * n + 1)], 
                  trans)
                c(b$geno, b$vino, b$conf)
            }, trans)
            genom = t(genom)
            confm = genom[, c((2 * n + 1):(3 * n))]
            vinom = genom[, c((n + 1):(2 * n))]
            genom = genom[, c(1:n)]
            conf[pseudoautosomalID, ] = confm
            vino[pseudoautosomalID, ] = vinom
            geno[pseudoautosomalID, ] = genom
        }
